      ...
      <div class="py-2">
        <form class="row" action="">
          ...
          <div class="col-md-8">
            ...
            $b = $_GET['b'];
            if ($b != '') {
              echo '<a class="btn btn-outline-dark border shadow-sm float-end my-2" href="../print/select_date.php?b=' . $_GET['b'] . '"  ><i class="bi bi-printer-fill fa-fw"> </i>  ไปหน้าพิมพ์ทั้งหมด</a>';
            }
            ?>
          </div>
        </form>
        ...
        <table class="table table-sm table-hover table-striped view ">
          ...
          <tbody>
            ...
            $stmt = $conn->prepare("SELECT  *,CASE 
             WHEN DATE_FORMAT('$b' , '%m' ) - DATE_FORMAT( start_date, '%m' ) - ( DATE_FORMAT('$b' , '00-%m-%d' )
             < DATE_FORMAT( start_date, '00-%m-%d' ) ) + 1 = 12 THEN  DATE_FORMAT('$b' , '%Y' ) - DATE_FORMAT( start_date, '%Y' ) - ( DATE_FORMAT('$b', '00-%m-%d' ) 
             < DATE_FORMAT( start_date, '00-%m-%d' ) ) + 1 
             WHEN DATE_FORMAT('$b' , '%m' ) - DATE_FORMAT( start_date, '%m' ) - ( DATE_FORMAT( '$b' , '00-%m-%d' ) 
                        < DATE_FORMAT( start_date, '00-%m-%d' ) ) = '-2' THEN DATE_FORMAT('$b' , '%Y' ) - DATE_FORMAT( start_date, '%Y' ) - ( DATE_FORMAT( '$b' , '00-%m-%d' ) 
                        < DATE_FORMAT( start_date, '00-%m-%d' ) )+1 ELSE  DATE_FORMAT('$b' , '%Y' ) - DATE_FORMAT( start_date, '%Y' ) - ( DATE_FORMAT( '$b', '00-%m-%d' ) 
                        < DATE_FORMAT( start_date, '00-%m-%d' ) ) END as strYear,
                        case when id in ('1','7') then 
                        DATE_FORMAT('$b' , '%m' ) - DATE_FORMAT( start_date, '%m' ) - ( DATE_FORMAT('$b' , '00-%m-%d' ) 
                        < DATE_FORMAT( start_date, '00-%m-%d' ) )
                        WHEN DATE_FORMAT('$b' , '%m' ) - DATE_FORMAT( start_date, '%m' )  = '-1'  THEN 0
                        WHEN DATE_FORMAT('$b' , '%m' ) - DATE_FORMAT( start_date, '%m' )  = '-2'  THEN 11
                        WHEN DATE_FORMAT('$b' , '%m' ) - DATE_FORMAT( start_date, '%m' ) - ( DATE_FORMAT('$b' , '00-%m-%d' ) 
                        < DATE_FORMAT( start_date, '00-%m-%d' ) ) + 1 = 12 THEN '0'
                        else DATE_FORMAT('$b' , '%m' ) - DATE_FORMAT( start_date, '%m' ) - ( DATE_FORMAT('$b' , '00-%m-%d' ) 
                        < DATE_FORMAT( start_date, '00-%m-%d' ) ) + 1 
                        END as strMonth  FROM user_group where active ='Y'");
            $stmt->execute();
