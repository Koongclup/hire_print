...
<body Onloadก="window.print();">
	...
	when round((TIMESTAMPDIFF(Month,start_date,'$isdate' )/12),1) > 10 	
		THEN SUBSTR(round((TIMESTAMPDIFF(Month,start_date,'$isdate' )/12),1),1,2) 
	when  round((TIMESTAMPDIFF(Month,start_date,'$isdate' )/12),1) < 10 
		THEN SUBSTR(round((TIMESTAMPDIFF(Month,start_date,'$isdate' )/12),1),1,1)
END as strY,
case 
	when round((TIMESTAMPDIFF(Month,start_date,'$isdate' )/12),1) > 10 	
		THEN SUBSTR(round((TIMESTAMPDIFF(Month,start_date,'$isdate' )/12),1),4,1) 
	when  round((TIMESTAMPDIFF(Month,start_date,'$isdate' )/12),1) < 10 
		THEN SUBSTR(round((TIMESTAMPDIFF(Month,start_date,'$isdate' )/12),1),3,1)
END as strM,
DATE_FORMAT('$isdate' , '%Y' ) - DATE_FORMAT( start_date, '%Y' ) - ( DATE_FORMAT('$isdate' , '00-%m-%d' ) 
    < DATE_FORMAT( start_date, '00-%m-%d' ) ) as strYear,
    DATE_FORMAT('$isdate' , '%m' ) - DATE_FORMAT( start_date, '%m' ) - ( DATE_FORMAT('$isdate' , '00-%m-%d' ) 
    < DATE_FORMAT( start_date, '00-%m-%d' ) ) as strMonth2 ,
    case when g.id in ('1','7') then 
    DATE_FORMAT('$isdate' , '%m' ) - DATE_FORMAT( start_date, '%m' ) - ( DATE_FORMAT('$isdate' , '00-%m-%d' ) 
    < DATE_FORMAT( start_date, '00-%m-%d' ) ) ELSE 
    DATE_FORMAT('$isdate' , '%m' ) - DATE_FORMAT( start_date, '%m' ) - ( DATE_FORMAT('$isdate' , '00-%m-%d' ) 
    < DATE_FORMAT( start_date, '00-%m-%d' ) ) + 1 END as strMonth 
        ...
        <div class="main-page">
            <div class="sub-page">
                ...
                <b>
                    ...
                    ......รวม.....
                    <b class=""><?= $r['strYear']?></b>.......ปี......<b class=""><?= $r['strMonth']; ?></b>.....เดือน..............วัน
                </p>
